...
LABEL maintainer="haxqer <haxqer666@gmail.com>" version="8.5.17"
 
ARG ATLASSIAN_PRODUCTION=confluence
ARG APP_NAME=confluence
ARG APP_VERSION=8.5.17
...
ENV JAVA_OPTS="-javaagent:${AGENT_PATH}/${AGENT_FILENAME} ${JAVA_OPTS} ${CATALINA_OPTS} -Dupm.plugin.upload.enabled=true"
...
RUN sed -i 's/<session-timeout>60<\/session-timeout>/<session-timeout>300<\/session-timeout>/g' ${CONFLUENCE_INSTALL}/confluence/WEB-INF/web.xml
